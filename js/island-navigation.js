...
function handleIslandNavbarScroll() {
  ...
  const scrollThreshold = 10; // Add threshold to prevent jittery behavior
  
  // Only process if scroll difference is significant enough
  if (Math.abs(scrollY - islandLastScrollY) < scrollThreshold && !islandIsRetracting) {
    return;
  }
...
// Initialize navbar state with throttling for better performance
